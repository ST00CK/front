...
const SignUpPage = () => {
    ...
    const handleCheckEmailCode = async () => {
        ...
        console.log("handleCheckEmailCode 함수 호출됨"); // 함수 호출 여부 확인
        try {
            console.log("입력된 인증코드:", inputEmailCode); // inputEmailCode 값을 콘솔에 출력
            console.log("입력된 이메일:", email); // email 값을 콘솔에 출력
            const response = await emailCheckMutation.mutateAsync({ email, authCode: inputEmailCode });
            console.log("서버 응답:", response); // 서버 응답 값을 콘솔에 출력
